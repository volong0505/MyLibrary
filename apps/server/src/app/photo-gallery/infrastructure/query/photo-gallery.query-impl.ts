...
export class PhotoGalleryQueryImplement implements PhotoGalleryQuery {
    ...
    async findAll (page: number, limit: number, sort_column: string, sort_type: "asc" | "desc"): Promise<FindPhotosResult> {
        const raw_data = await this.model.find({}).skip((page - 1) * limit).limit(limit).sort({ sort_column: sort_type});
