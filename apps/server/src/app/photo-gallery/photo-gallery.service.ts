...
import { FavoringPhotosRequest, FavoringPhotosResponse, GetDetailPhotoRequest, GetDetailPhotoResponse, GetListPhotosRequest, GetListPhotosResponse } from "@my-library/api-interfaces";
...
export class PhotoGalleryService {
    ...
    async detailPhoto(dto: GetDetailPhotoRequest): Promise<GetDetailPhotoResponse> {
        const model: FindPhotoByNameResult = await this.queryBus.execute(new GetDetailPhotoQuery(dto));
        await this.commandBus.execute(new ViewedPhotoCommand({file_name: model.Information.FileName}));
        return {
    ...
    generatingFolder(): void {
        if (!fs.existsSync(photo_gallery_folder)) fs.mkdirSync(photo_gallery_folder, { recursive: true});
        if (!fs.existsSync(photo_gallery_resize_folder)) fs.mkdirSync(photo_gallery_resize_folder, { recursive: true});
    }
}