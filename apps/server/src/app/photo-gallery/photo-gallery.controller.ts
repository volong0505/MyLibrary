import { FavoringPhotosRequest, FavoringPhotosResponse, GetDetailPhotoRequest, GetDetailPhotoResponse, GetListPhotosRequest, GetListPhotosResponse, PHOTO_GALLERY_FAVORING_PHOTOS_API, PHOTO_GALLERY_GET_DETAIL_API, PHOTO_GALLERY_GET_LIST_API, PHOTO_GALLERY_GET_RESIZE_API, UPLOAD_PHOTOS_API } from "@my-library/api-interfaces";
import { BadRequestException, Body, Controller, Get, Post, Put, Query, Res, UploadedFiles, UseInterceptors } from "@nestjs/common";
...
export class PhotoGalleryController {
    ...
    getList(@Query() params: GetListPhotosRequest): Promise<GetListPhotosResponse> {
        return this.service.listPhotos(params)
    }

    @Get(PHOTO_GALLERY_GET_DETAIL_API)
    getDetail(@Query() params: GetDetailPhotoRequest): Promise<GetDetailPhotoResponse> {
    ...
    getImage(@Query() params: any, @Res() response) {
        const { file_name } = params;
            const resize_file_path = photo_gallery_resize_folder + '/resize_'+ file_name;
